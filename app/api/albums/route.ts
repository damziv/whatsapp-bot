...
export async function GET(req: NextRequest) {
  ...
  // Best-effort media counts (beta-safe; can be optimized later)
  ...
  return NextResponse.json({
    albums: rows.map((r) => {
      const bride = r.events?.profiles?.bride_name ?? null;
      const groom = r.events?.profiles?.groom_name ?? null;
      const owner_email = r.events?.profiles?.owner_email ?? null;
      const type = (r.events as any)?.type ?? null;
...
export async function DELETE(req: NextRequest) {
  ...
  // Fetch album + profile_id via event (we delete profile to cascade)
  ...
  if (profileId) {
    ...
    // fallback: delete just the album
