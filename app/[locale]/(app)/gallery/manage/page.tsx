...
function ManageInner() {
  ...
  const title = useMemo(() => {
    if (!code) return t('manageTitle');
    return meta?.label ? t('managePrefix', { label: meta.label }) : t('manageAlbum', { code });
  }, [code, meta?.label, t]);
  ...
  const submitPin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(out?.error || t('invalidPin'));

      localStorage.setItem(LS_TOKEN_KEY(code), out.token);
      setToken(out.token);
      setPin('');
    } catch (e: any) {
      setErr(e?.message || t('pinFailed'));
    } finally {
      setPinLoading(false);
    }
  };
  ...
  const downloadAllZip = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error((out as any)?.error || t('downloadFailed'));
      }
      ...
      setErr(e?.message || t('downloadFailed'));
    } finally {
      ...
      <div className="mx-auto w-full max-w-5xl">
          ...
          <div>
            ...
            {meta?.start_at && meta?.end_at && (
              <div className="mt-1 text-xs text-neutral-600 dark:text-neutral-300">
                {t('uploadWindow')}: {new Date(meta.start_at).toLocaleString()} → {new Date(meta.end_at).toLocaleString()}
              </div>
            )}
          </div>
        ...
        {!token && (
          <div className="mb-6 rounded-2xl border border-black/5 bg-white p-6 shadow-card dark:border-white/10 dark:bg-white/5">
            <h2 className="text-lg font-semibold">{t('enterPin')}</h2>
