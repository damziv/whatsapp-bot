    ...
    <main className="min-h-screen bg-gradient-to-b from-brand-50 to-white px-4 py-10 dark:from-neutral-900 dark:to-neutral-950">
      <div className="mx-auto w-full max-w-3xl space-y-6">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-semibold tracking-[-0.02em]">{t('settingsTitle')}</h1>
        ...
        <section className="rounded-2xl border border-black/5 bg-white p-6 shadow-card dark:border-white/10 dark:bg-white/5">
          <h2 className="text-lg font-semibold tracking-[-0.01em]">{t('account')}</h2>

          <div className="mt-4 grid gap-3 text-sm">
            <Row label={t('role')} value={isPhotographer ? t('roleUser') : t('roleNone')} />
            <Row label={t('status')} value={isActive ? t('active') : t('inactive')} />
            <Row
              label={t('userId')}
              value={
        ...
        {/* Plan card */}
        <section className="rounded-2xl border border-black/5 bg-white p-6 shadow-card dark:border-white/10 dark:bg-white/5">
          <h2 className="text-lg font-semibold tracking-[-0.01em]">{t('plan')}</h2>

          {portal && !('error' in portal) ? (
            <div className="mt-4 grid gap-3 text-sm">
              <Row label={t('usedAlbums')} value={usageText || '—'} />
              <Row label={t('totalAvailable')} value={String(portal.photographer.quota_yearly)} />
              <Row label={t('period')} value={periodText || '—'} />
            </div>
          ) : (
        ...
        {/* Support card */}
        <section className="rounded-2xl border border-black/5 bg-white p-6 shadow-card dark:border-white/10 dark:bg-white/5">
          <h2 className="text-lg font-semibold tracking-[-0.01em]">{t('support')}</h2>
