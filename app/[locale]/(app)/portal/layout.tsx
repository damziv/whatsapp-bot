  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!res.ok || ('error' in payload && payload.error)) {
        setStatus('denied');
        setMsg(('error' in payload && payload.error) || t('accessDenied'));
        return;
      }

      if (!('isPhotographer' in payload) || payload.isPhotographer !== true) {
        setStatus('denied');
        setMsg(t('accessNotEnabled'));
        return;
      }

      if (!payload.isActive) {
        setStatus('inactive');
        setMsg(t('accountInactive'));
        return;
      }
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-b from-brand-50 to-white dark:from-neutral-900 dark:to-neutral-950">
      ...
      <aside className="hidden w-64 border-r border-black/5 bg-white p-4 dark:border-white/10 dark:bg-neutral-900 md:block">
        <div className="mb-6 text-lg font-semibold">📸 {t('brand')}</div>
        <nav className="space-y-1">
          <NavItem href="/portal/albums" label={t('navAlbums')} />
          <NavItem href="/portal/create" label={t('navCreate')} />
          <NavItem href="/portal/settings" label={t('navSettings')} />
        </nav>
