...
export default function PortalCreatePage() {
  ...
  const [lang, setLang] = useState(''); // '' = auto, 'hr', 'en'
  ...
  const onCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(('error' in out && out.error) || t('failedCreate'));
  ...
  return (
    <>
      <div className="mx-auto w-full max-w-4xl">
        <div className="mb-6">
          <h1 className="text-2xl font-semibold tracking-[-0.02em]">{t('createTitle')}</h1>
          ...
          <form onSubmit={onCreate} className="grid gap-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="grid gap-1.5">
                <label className="text-sm font-medium">{t('brideName')}</label>
                <input
                  value={bride}
              ...
              <div className="grid gap-1.5">
                <label className="text-sm font-medium">{t('groomName')}</label>
                <input
                  value={groom}
            ...
            <div className="grid gap-1.5">
              <label className="text-sm font-medium">{t('langLabel')}</label>
              <select
                value={lang}
        ...
        <div className="mt-6 rounded-2xl border border-black/5 bg-white p-5 text-sm shadow-card dark:border-white/10 dark:bg-white/5">
          <div className="font-semibold">{t('whatsNext')}</div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-card dark:bg-neutral-900">
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="text-lg font-semibold">{t('albumCreated')}</h3>
                <p className="mt-1 text-sm text-neutral-600 dark:text-neutral-300">
                {t('albumCreatedDesc')}
                </p>
              </div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-4">
              ...
              <div className="min-w-0 flex-1 space-y-3">
                <div className="text-sm">
                  <div className="text-xs text-neutral-500">{t('guestQr')}</div>
                ...
                <div className="text-sm">
                  <div className="text-xs text-neutral-500">{t('publicGallery')}</div>
                ...
                <div className="text-sm">
                  <div className="text-xs text-neutral-500">{t('manageGallery')}</div>
                ...
                <div className="text-sm">
                  <div className="text-xs text-neutral-500">{t('albumCode')}</div>
